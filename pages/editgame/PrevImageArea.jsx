...
import { FormDiv, TypeTagsInput, PrevImageWrap } from '../../styles/uploadForms'
...
const PrevImageArea = ({ id, tags, update, gameName }) => {
  ...
  if(selectFile) {
    ...
    ( tagMainValue.value = selectFile.tags.main, tagSubValue.value = selectFile.tags.sub, tagOptionValue.value = selectFile.tags.optional) :
    ( tagMainValue.value = '', tagSubValue.value = '', tagOptionValue.value = '',
      tagMainValue.placeholder = main, tagSubValue.placeholder = sub, tagOptionValue.placeholder = optional);
  ...
  const postFiles = async () => {
      ...
      await Promise.all(toPostFiles.map(async (toPostFile) => {
        ...
        const response = await axios.post(`/images/${id}`,{...toPostFile, imageUrl: s3Url.data});
        console.log(response.data.data);
      }));
    

    const exist = [...existImage];
    setExist(exist);
    setSelectImage(null);
    setChecked(0);
    setFiles([]);
  ...
  return (
    <FormDiv>
      ...
      <InputWrap>
        <TypeTagsInput id='main' type="text" placeHolder={main}/>
        <TypeTagsInput id='sub' type="text" placeHolder={sub}/>
        <TypeTagsInput id='option' type="text" placeHolder={optional}/>
        {isTagsAvailable ? <button onClick={() => patchFiles()}>수정</button>: (checked == fileLength ? <button onClick={() => postFiles()}>업로드</button> : <button onClick={() => setFileTags(selectFile)}>추가</button>)}
